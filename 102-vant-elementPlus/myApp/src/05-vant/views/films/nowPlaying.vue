...
<template>
    <div>
        ...
        <van-list v-model:loading="loading" :finished="finished" finished-text="没有更多了..." @load="onLoad" offset="10"
            ...
            <van-cell v-for="item in dataList" :key="item.filmId" @click="navigateHandler(item.filmId)"
                style="height: 100px;">
                <img :src="item.poster" ref="电影" style="width: 60px;left: 0px;position: absolute;left: 0px;">
                <div>{{ item.name }}</div>
            </van-cell>
        </van-list>
    </div>
...
const onLoad = async () => {
    ...
    // 使用异步的方式
    const res = await axios.get(`https://m.maizuo.com/gateway?cityId=440300&pageNum=${pageNum.value}&pageSize=10&type=1&k=4144633`, {
        headers: {
            'X-Client-Info': '{ "a": "3000", "ch": "1002", "v": "5.2.1", "e": "17116912502046590571249665" }',
            'X-Host': 'mall.film-ticket.film.list'
        }
